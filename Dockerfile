...
RUN opam depext -uiyvj 4 irmin-watcher git-unix irmin.0.12.0 irmin-unix.0.12.0 cmdliner astring fmt logs ptime fpath
RUN opam pin add -y git-delve https://github.com/avsm/git-delve.git
...
RUN opam config exec -- git-delve -d _repos > scripts/loc.txt
